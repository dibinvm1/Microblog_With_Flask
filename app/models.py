...
class User(UserMixin,db.Model):
    ...
    comments = db.relationship('Comment',backref = 'commentAuthor' , lazy = 'dynamic', cascade = "all, delete, delete-orphan")
    ...
    def new_messages(self):
        ''' get the new messages recieved after the last_read_time '''
        last_read_time = self.last_message_read_time or datetime(1900,1,1)
        return Message.query.filter_by(recipient = self).filter(
            Message.timestamp > last_read_time).count()
    
    def last_message(self):
        '''gets the last message recieved or sent'''
        return Message.query.filter((Message.sender_id == self.id) | (Message.recipient_id == self.id)).order_by(Message.timestamp.desc()).first()
...
class Post(SearchableMixin, db.Model):
    ...
    comments = db.relationship('Comment',backref = 'commentedPost' , lazy = 'dynamic', cascade = "all, delete, delete-orphan")
